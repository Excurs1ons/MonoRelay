    ...
    <div v-else class="settings-grid">
      ...
      <div class="card">
        ...
        <div class="form-group">
          ...
          <p class="help-text">在路由层统一补齐或覆写请求参数。</p>
        </div>
        <div v-if="config.model_routing.global_params.enabled">
          <div class="form-group">
            ...
            <select v-model="config.model_routing.global_params.mode" class="form-input">
              <option value="default">默认 (仅当请求缺失该参数时补齐)</option>
          ...
          <p class="help-text">支持数字、布尔（true/false）和字符串。例如: max_tokens: 4096, stream: true</p>
        </div>
      </div>
...
const config = ref({
  ...
  model_routing: {
    global_params: { enabled: false, mode: 'default', params: {} }
  },
...
async function fetchFullConfig() {
  ...
  try {
    ...
    if (!data.model_routing.global_params) {
        data.model_routing.global_params = { enabled: false, mode: 'default', params: {} }
    }
    config.value = data
...
function addParam() {
  config.value.model_routing.global_params.params['new_param_' + Date.now()] = ''
}

function removeParam(key) {
  delete config.value.model_routing.global_params.params[key]
}

function generateRandomKey() {
  config.value.server.access_key = crypto.randomUUID()
}
