...
async def handle_messages(
    ...
    if is_stream:
        ...
        return await _non_stream_messages(
            provider_cfg, url, headers, body, key, key_manager, provider_name,
            resolved_model, original_model, request_logger, start_time, stats_tracker,
            client_ip, user_agent, downstream_request
        )
...
async def _stream_messages(
    ...
    while attempt <= provider_cfg.retry.max_retries:
        try:
            ...
            log_parts = [f"Anthropic娴佸紡 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name}"]
            ...
            if tokens_in is not None or tokens_out is not None: log_parts.append(f"鎬籺oken={total}")
            ...
            await request_logger.log_request(
                ...
                response_preview=resp_preview, client_ip=client_ip, user_agent=user_agent, downstream_request=downstream_request, downstream_response=json.dumps(result, ensure_ascii=False)
                request_full=json.dumps(body, ensure_ascii=False, indent=2),
            ...
            key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
            elapsed = time.time() - start_time
            logger.error(f"Anthropic娴佸紡澶辫触 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 閿欒={e}")
            await request_logger.log_request(
                model=resolved_model,
                provider=provider_name,
                key_label=key.key.label,
        ...
        try:
            async with httpx.AsyncClient(timeout=httpx.Timeout(provider_cfg.timeout, connect=10.0)) as client:
                ...
                if resp.status_code >= 400:
                    ...
                    key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
                    logger.error(f"Anthropic閿欒{status_code} | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name}")
                    await request_logger.log_request(
                        model=resolved_model, provider=provider_name,
                        key_label=key.key.label, status_code=status_code,
                        latency_ms=round(elapsed * 1000, 2),
                ...
                log_parts = [f"Anthropic闈炴祦寮?| 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name}"]
                if tokens_in is not None: log_parts.append(f"杈撳叆token={tokens_in}")
                if tokens_out is not None: log_parts.append(f"杈撳嚭token={tokens_out}")
                total = (tokens_in or 0) + (tokens_out or 0)
                if tokens_in is not None or tokens_out is not None: log_parts.append(f"鎬籺oken={total}")
            ...
            key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
            logger.error(f"Anthropic澶辫触 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 閿欒={e}")
            stats_tracker.record_request(provider_name, resolved_model, success=False)
...
async def handle_anthropic_models(
    ...
    start_time = time.time()
    logger.info(f"Anthropic models璇锋眰 | 鎻愪緵鍟?{provider_name}")
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(provider_cfg.timeout, connect=10.0)) as client:
        try:
            ...
            if resp.status_code >= 400:
                key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
                stats_tracker.record_request(provider_name, "models", success=False)
                logger.error(f"Anthropic models閿欒{resp.status_code} | 鎻愪緵鍟?{provider_name}")
                return resp.json()
            key_manager.report_success(key, 0)
            logger.info(f"Anthropic models | 鎻愪緵鍟?{provider_name} | 鑰楁椂={round(elapsed * 1000, 2)}ms")
            await request_logger.log_request(
                model="models",
                provider=provider_name,
                key_label=key.key.label,
            ...
            key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
            stats_tracker.record_request(provider_name, "models", success=False)
            logger.error(f"Anthropic models澶辫触 | 鎻愪緵鍟?{provider_name} | 閿欒={e}")
...
async def handle_anthropic_messages_beta(
    ...
    logger.info(f"Anthropic messages beta璇锋眰 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name}")
...
async def _stream_anthropic_messages_beta(
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(provider_cfg.timeout, connect=10.0)) as client:
        try:
            ...
            elapsed = time.time() - start_time
            logger.info(f"Anthropic messages beta stream瀹屾垚 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 鑰楁椂={round(elapsed * 1000, 2)}ms")
            await request_logger.log_request(
                model=resolved_model,
                provider=provider_name,
                key_label=key.key.label,
            ...
            key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
            elapsed = time.time() - start_time
            logger.error(f"Anthropic messages beta stream澶辫触 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 閿欒={e}")
            await request_logger.log_request(
                model=resolved_model,
                provider=provider_name,
                key_label=key.key.label,
...
async def _non_stream_anthropic_messages_beta(
    ...
    async with httpx.AsyncClient(timeout=httpx.Timeout(provider_cfg.timeout, connect=10.0)) as client:
        try:
            ...
            logger.info(f"Anthropic messages beta non-stream瀹屾垚 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 鑰楁椂={round(elapsed * 1000, 2)}ms")
            await request_logger.log_request(
                model=resolved_model,
                provider=provider_name,
                key_label=key.key.label,
            ...
            key_manager.report_failure(provider_name, key, provider_cfg.rate_limit_cooldown)
            elapsed = time.time() - start_time
            logger.error(f"Anthropic messages beta non-stream澶辫触 | 妯″瀷={resolved_model} | 鎻愪緵鍟?{provider_name} | 閿欒={e}")
            await request_logger.log_request(
                model=resolved_model,
                provider=provider_name,
                key_label=key.key.label,
